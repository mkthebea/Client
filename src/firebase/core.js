...
// WARNING: 가능하면 해당 함수로 유효성 체크하는 getQuerySnapShot 이외에 다른 곳에서 collection에 직접 접근하지 않기
...
// collectionName으로
async function getQuerySnapshot(collectionName) {
...
async function getDocsOf(collectionName) {
  const snapshot = await getQuerySnapshot(collectionName);
  const ret = [];
